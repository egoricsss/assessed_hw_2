...
class Graph {
 private:
  ...
  void ClearProcessingStack() { std::stack<VertexId>().swap(processing_order_); }
  ...
  void DepthFirstVisit(VertexId vertex, std::vector<char>& visited, PreVisit&& pre_visit, PostVisit&& post_visit) {
    visited[vertex] = true;
    pre_visit(vertex);
  ...
  static AdjacencyList ParseGraphBuffer(const std::string& buffer) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      while (ptr < end && (u = ParseInt(&ptr)) != -1) {
        ...
        if (u < 0 || u >= n) {
          throw std::runtime_error("Invalid adjacent vertex index");
        }
 ...
 public:
  Graph() = default;
  explicit Graph(AdjacencyList&& adjacency) : adjacency_(std::move(adjacency)) {}
...
int main() {
  try {
    ...
    std::unordered_map<VertexId, std::vector<VertexId>> component_map;
